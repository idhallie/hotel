...
module HotelSystem
  class Hotel
    ...
    def initialize(rooms: [], reservations: [], blocks: [])
      ...
      20.times do |i|
        rooms << HotelSystem::Room.new(id:(i+1).to_i)
      end
    end
    ...
    def make_block(start_date:, end_date:, num_rooms:, discount:)
      ...
      num_rooms.times do |count|
        reservation_id = reservations.length + 1
        block_reservation = HotelSystem::Reservation.new(id: reservation_id, room: block_avail_rooms[count], start_date: start_date, end_date: end_date, discount: discount, block_res_taken: false)
      ...
      new_block = HotelSystem::Block.new(id: block_id, reservations: block_res_array)
      blocks << new_block
